...
def _ensure_recent_memory_list():
    if "memory_recent" not in st.session_state:
        st.session_state.memory_recent = []  # list[dict]: {userid, chapterid, key, value, ts}

def _push_recent_memory(userid: str, chapterid: str | None, key: str, value: object):
    _ensure_recent_memory_list()
    item = {
        "userid": userid,
        "chapterid": chapterid or "",
        "key": key,
        "value": value,
        "ts": datetime.now().isoformat(timespec="seconds"),
    }
    # Dedup by (userid, chapterid, key)
    st.session_state.memory_recent = [
        x for x in st.session_state.memory_recent
        if not (x.get("userid") == item["userid"] and x.get("chapterid") == item["chapterid"] and x.get("key") == item["key"])
    ]
    st.session_state.memory_recent.insert(0, item)
    st.session_state.memory_recent = st.session_state.memory_recent[:20]
...
with st.sidebar:
    ...
    user_id = st.text_input("User ID", value="Jitender Bajaj", key="user_id_input")
...
with tab3:
    ...
    with col1:
        ...
        memuser = st.text_input("User ID", value="student_001", key="memstore_user")
        memchapter = st.text_input("Chapter ID (optional)", value="", key="memstore_chapter")
        memkey = st.text_input("Key", value="learning_style", key="memstore_key")

        valuetype = st.radio(
            "Value type",
            options=["Text", "JSON"],
            horizontal=True,
            key="memstore_value_type",
            help="Use JSON for structured preferences like {'learning_style':'visual','pace':'slow'}.",
        )

        if valuetype == "JSON":
            raw = st.text_area(
                "Value (JSON)",
                value='{"learning_style": "visual"}',
                height=120,
                key="memstore_value_json",
            )
            parsed_value = None
            parse_error = None
            try:
                parsed_value = json.loads(raw) if raw.strip() else None
            except Exception as e:
                parse_error = str(e)
            ...
            memvalue = parsed_value
            can_store = bool(memuser.strip() and memkey.strip() and parsed_value is not None and not parse_error)
        else:
            memvalue_text = st.text_input("Value", value="visual learner", key="memstore_value_text")
            memvalue = memvalue_text
            can_store = bool(memuser.strip() and memkey.strip() and str(memvalue_text).strip())

        memttl = st.number_input("Retention (days)", value=90, min_value=1, key="memstore_ttl")

        if st.button("Store", key="memstore_btn", disabled=not can_store, use_container_width=True):
            # payload = {
            #     "userid": memuser.strip(),
            #     "chapterid": memchapter.strip() or None,
            #     "key": memkey.strip(),
            #     "value": memvalue,
            #     "retentionttldays": int(memttl),
            # }
            payload = {
                "user_id": memuser.strip(),
                "key": memkey.strip(),
                "value": memvalue,  # for Text mode, keep it as string
                "retentionttldays": int(memttl),
            }

            if memchapter.strip():
                payload["chapterid"] = memchapter.strip()

            try:
                with st.spinner("Storing memory..."):
                    resp = requests.post(f"{BACKEND_URL}/memory/put", json=payload, timeout=10)
                if resp.status_code == 200:
                    st.success("Memory stored successfully!")
                    _push_recent_memory(memuser.strip(), memchapter.strip() or None, memkey.strip(), memvalue)
                    if st.session_state.get("debugui", False):
                        with st.expander("Debug: /memory/put response", expanded=False):
                            st.json(resp.json() if resp.content else {"status_code": resp.status_code})
                else:
                    st.error(f"Store failed: HTTP {resp.status_code}")
                    try:
                        st.json(resp.json())
                    except Exception:
                        st.code(resp.text)
                    if st.session_state.get("debugui", False):
                        with st.expander("Debug: /memory/put error", expanded=True):
                            st.code(resp.text)
            except Exception as e:
                st.error(f"Error: {e}")
    ...
    with col2:
        ...
        getuser = st.text_input("User ID", value="student_001", key="memget_user")
        getchapter = st.text_input("Chapter ID (optional)", value="", key="memget_chapter")

        recent = st.session_state.memory_recent
        recent_choices = ["(type manually)"] + [
            f'{x["key"]}  —  user={x["userid"]}  ch={x["chapterid"] or "-"}'
            for x in recent
        ]
        pick = st.selectbox("Recent keys", options=recent_choices, key="memget_recent_pick")

        default_key = "learning_style"
        if pick != "(type manually)":
            idx = recent_choices.index(pick) - 1
            chosen = recent[idx]
            default_key = chosen.get("key") or default_key

        getkey = st.text_input("Key", value=default_key, key="memget_key")

        can_get = bool(getuser.strip() and getkey.strip())

        if st.button("Retrieve", key="memget_btn", disabled=not can_get, use_container_width=True):
            try:
                params = {"user_id": getuser.strip(), "key": getkey.strip()}
                if getchapter.strip():
                    params["chapterid"] = getchapter.strip()

                with st.spinner("Retrieving memory..."):
                    resp = requests.get(f"{BACKEND_URL}/memory/get", params=params, timeout=10)

                if resp.status_code == 200:
                    result = resp.json() if resp.content else {}
                    if result.get("status") == "success":
                        st.success("Memory found.")
                        value = result.get("value")
                        # Pretty display: if dict/list show JSON; else show text
                        if isinstance(value, (dict, list)):
                            st.json(value)
                        else:
                            st.write(value)

                        _push_recent_memory(getuser.strip(), getchapter.strip() or None, getkey.strip(), value)

                        if result.get("storedat"):
                            st.caption(f"Stored at: {result.get('storedat')}")

                        if st.session_state.get("debugui", False):
                            with st.expander("Debug: /memory/get response", expanded=False):
                                st.json(result)
                    else:
                        st.warning("Memory not found.")
                        if st.session_state.get("debugui", False):
                            with st.expander("Debug: /memory/get response", expanded=True):
                                st.json(result)
                else:
                    st.error(f"Retrieve failed: HTTP {resp.status_code}")
                    if st.session_state.get("debugui", False):
                        with st.expander("Debug: /memory/get error", expanded=True):
                            st.code(resp.text)
            except Exception as e:
                st.error(f"Error: {e}")
    
    st.divider()
    st.subheader("Recent Memory Activity (this session)")
    st.caption("This is UI-only and resets when Streamlit reloads.")

    recent = st.session_state.get("memory_recent", [])
    if not recent:
        st.info("No recent memory operations yet. Use Store or Retrieve above.")
    else:
        # Flatten for display
        rows = []
        for r in recent:
            rows.append({
                "ts": r.get("ts"),
                "userid": r.get("userid"),
                "chapterid": r.get("chapterid") or "",
                "key": r.get("key"),
                "value_preview": (json.dumps(r.get("value"))[:120] if isinstance(r.get("value"), (dict, list)) else str(r.get("value"))[:120]),
            })

        st.dataframe(rows, use_container_width=True, hide_index=True)

        # Pick one row to load back into forms
        pick_labels = [
            f'{x.get("ts","")} | user={x.get("userid")} ch={x.get("chapterid") or "-"} | {x.get("key")}'
            for x in recent
        ]
        picked = st.selectbox("Select an entry to load", options=pick_labels, key="mem_recent_pick_load")
        picked_idx = pick_labels.index(picked)
        item = recent[picked_idx]

        c1, c2, c3 = st.columns([1, 1, 2])

        with c1:
            if st.button("Load into Retrieve", use_container_width=True, key="mem_load_retrieve"):
                st.session_state["memget_user"] = item.get("userid") or ""
                st.session_state["memget_chapter"] = item.get("chapterid") or ""
                st.session_state["memget_key"] = item.get("key") or ""

        with c2:
            if st.button("Load into Store", use_container_width=True, key="mem_load_store"):
                st.session_state["memstore_user"] = item.get("userid") or ""
                st.session_state["memstore_chapter"] = item.get("chapterid") or ""
                st.session_state["memstore_key"] = item.get("key") or ""

                v = item.get("value")
                if isinstance(v, (dict, list)):
                    st.session_state["memstore_value_type"] = "JSON"
                    st.session_state["memstore_value_json"] = json.dumps(v, indent=2)
                else:
                    st.session_state["memstore_value_type"] = "Text"
                    st.session_state["memstore_value_text"] = "" if v is None else str(v)

        with c3:
            st.download_button(
                "Download recent activity (JSON)",
                data=json.dumps(recent, indent=2),
                file_name="memory_recent_activity.json",
                mime="application/json",
                use_container_width=True,
                key="mem_download_recent",
            )
